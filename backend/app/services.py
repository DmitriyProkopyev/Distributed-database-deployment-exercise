...
async def get_doc(doc_id: str) -> dict:
    ...
    await kafka_client.send_message(
        settings.KAFKA_TOPIC_BACKEND_TO_DB,
        {"action": "read_one", "doc_id": doc_id, "request_id": request_id}
    )
    response = await wait_for_kafka_response(request_id)
    if "error" in response:
        raise HTTPException(500, response["error"])
...
async def get_docs(skip: int = 0, limit: int = 100) -> list[dict]:
    ...
    await kafka_client.send_message(
        settings.KAFKA_TOPIC_BACKEND_TO_DB,
        {"action": "read_many", "skip": skip, "limit": limit, "request_id": request_id}
    )
    response = await wait_for_kafka_response(request_id)
    if "error" in response:
        raise HTTPException(500, response["error"])
...
async def health_check() -> dict:
    try:
        await kafka_client.producer.send(settings.KAFKA_TOPIC_BACKEND_TO_DB, {"action": "health_check"})
